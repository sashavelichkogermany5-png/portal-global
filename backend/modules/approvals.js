    ...
    for (const row of rows) {
      ...
      if (actionType === 'require_approval') {
        ...
        const result = await dbRun(
          ...
          [workspaceId, String(action.approvalType || 'rule'), entity, entityId, safeJsonStringify({ event, payload, rule: row.name })]
        );
        await logAudit({
          workspaceId,
      ...
      if (actionType === 'tag') {
        ...
        if (tag && payload?.entity?.type === 'lead' && entityId) {
          const leadRow = await dbAll('SELECT tags_json FROM leads WHERE id = ? AND tenant_id = ?', [entityId, workspaceId]);
          const existingTags = safeJsonParse(leadRow?.[0]?.tags_json, []) || [];
      ...
      if (actionType === 'enqueue_job') {
        const job = action.job && typeof action.job === 'object' ? action.job : { type: 'httpPing', url: action.url };
        const result = await dbRun(
          ...
          [workspaceId, actor?.userId || 0, 'LocalAgent', safeJsonStringify(job)]
        );
        await logAudit({
          workspaceId,
          actorType: actor?.actorType || 'user',
...
const createApprovalsRouter = ({
  ...
  router.get('/', requireBusinessPlan, requireViewer, async (req, res) => {
    try {
  ...
  router.post('/decide', requireBusinessPlan, requireAdmin, async (req, res) => {
    try {
...
const createAutomationsRouter = ({
  ...
  router.post('/', requireBusinessPlan, requireAdmin, async (req, res) => {
    try {
      ...
      const result = await dbRun(
        `INSERT INTO automations (workspace_id, name, enabled, trigger, config_json, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, datetime('now'), datetime('now'))`,
        [req.tenantId, name, 1, trigger, safeJsonStringify(config)]
      );
      await logAudit({
        workspaceId: req.tenantId,
        actorType: 'user',
