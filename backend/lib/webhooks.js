...
const createWebhooksRouter = ({
    ...
    publicRouter.post('/in/:id', async (req, res) => {
        try {
            const id = Number(req.params.id);
            const providedSecret = req.headers['x-portal-secret'] ? String(req.headers['x-portal-secret']).trim() : '';
            ...
            if (!row || !row.enabled || !row.secret_hash) return sendError(res, 404, 'Not Found', 'Webhook not found');
