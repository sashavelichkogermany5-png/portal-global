...
const resultsPath = path.join(root, 'ops', 'ga', 'results-stats.json');
const backupDir = path.join(root, 'ops', 'ga', 'backup');
const backupPath = path.join(backupDir, 'stats-cache-env.json');
...
const apply = () => {
  ...
  const newValues = {
    STATS_CACHE_TTL_MS: String(best.STATS_CACHE_TTL_MS),
    USAGE_CACHE_TTL_MS: String(best.USAGE_CACHE_TTL_MS)
  };
  ...
  writeEnv(bom, lines);
  console.log('[ga] Applied stats cache values to .env');
